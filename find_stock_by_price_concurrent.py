...
def load_single_stock_data(csv_file: Path) -> tuple[str, pd.DataFrame] | None:
    """加载单个股票数据文件"""
    try:
...
def load_stock_data_concurrent(
    data_dir: Path, max_workers: int | None = None
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # 提交所有任务
        future_to_file = {
            executor.submit(load_single_stock_data, csv_file): csv_file for csv_file in csv_files
        }
...
def find_by_price_single_stock(
    ...
    for _, row in matching_rows.iterrows():  #
        results.append(
            (stock_code, row[price_type], pd.to_datetime(row["date"]).strftime("%Y-%m-%d"))
        )
...
def find_by_price_concurrent(
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # 提交所有任务
        future_to_stock = {
            executor.submit(search_func, stock_item): stock_item[0] for stock_item in stock_data
        }
...
def print_results(results: list[tuple[str, float, str]], price_type: str) -> None:
    """打印搜索结果"""
    if not results:
        print("未找到符合条件的股票")
        return

    price_type_name = {"close": "收盘价", "high": "最高价", "low": "最低价"}.get(
        price_type, price_type
    )
